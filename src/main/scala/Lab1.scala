...
object Lab1 {
    ...
    def main(args: Array[String]) {
        ...
        val spark: SparkSession = SparkSession.builder.appName("Lab 1").getOrCreate
        ...
        val dataSetCities = spark
            .read
            .schema(orcSchema)
            .orc("./data/netherlands-latest.orc")
            .as[OrcSchemaClass]
        ...
        // [place, num_evacuees, h3index, lat, lon, place, elevation]
        ...
        println("-----------------------------------------------------------------------")
        println("Population that need to move: " + unsafePlaces.agg(sum("num_evacuees")).first.get(0)+ " at sealevel " + SEALEVEL)
        println("-----------------------------------------------------------------------")
        ...
        // Show dataset to force scala to compute and unpersist dataSetAdequate because it's not needed anymore
        dataSetHarbour.show(false)
        dataSetAdequate.unpersist()
        ...
        // Force scala to compute and unpersist to manage cache
        goodDataSet.show(40, false)
        unsafePlaces.unpersist()
        safeCities.unpersist()
        ...
        goodDataSet.unpersist()
        dataSetHarbour.unpersist()
        ...
        val dataSetExcellent = dataSetFinalJoin
            ...
            // Add the waterworld evacuees here so that we one dataSet can write to orc files
            .union(waterWorldEvacuees)
            .cache()

        // See results to verify they look ok and calculate the total number of evacuees for that sealevel rise.
        dataSetExcellent.show(false)
        dataSetFinalJoin.unpersist()
        ...
        // See results to verify and clean cache
        dataSet13.show(false)
        dataSetExcellent.unpersist()

        // write second orc file
        dataSet13
            .write
            .mode("overwrite")
            .orc("./output/excellent2.orc")
